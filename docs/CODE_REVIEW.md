...
| The Settings storage actions refuse a claim set whose *sidecar* read failed | The hike fetch is a closure and can be made to throw, and `SettingsStorageFailureTests` does. `HikeLocalState`'s own fetch is not injectable and a `ModelContext` cannot be made to fail on demand, so that half is refused by type — `Hike.tileClaim()` throws where the passthroughs answer "no tiles" — rather than watched |
