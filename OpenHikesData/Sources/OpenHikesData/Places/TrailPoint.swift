...
public final class TrailPoint {
    ...
    /// ``symbolID`` is.
    public var osmElementType: String = ""
    /// The element's id, `0` alongside an empty ``osmElementType``.
    public var osmElementID: Int64 = 0
    ...
    public var osmFacts: [TrailPlaceFact] = []
    ...
    public init(hikeID: UUID, place: TrailPlace, createdAt: Date = .now) {
        self.hikeID = hikeID
        id = place.id
...
extension TrailPoint {
    ...
    public func apply(name: String, symbol: TrailPlaceSymbol?, note: String) {
...
extension Hike {
    ...
    public var orderedPlaces: [TrailPlaceRow] {
    ...
    public func replacePlaces(with places: [TrailPlace], in context: ModelContext, now: Date = .now) {
    ...
    @discardableResult public func addPlace(_ place: TrailPlace, in context: ModelContext, now: Date = .now) -> Bool {
    ...
    private static func holds(_ place: TrailPlace, among held: [TrailPlace]) -> Bool {
        isSameElement(place, asAnyOf: held) || held.contains { existing in
            RouteGeometry.distanceMeters(from: existing.clCoordinate, to: place.clCoordinate)
                <= TrailPlace.alreadyMarkedMeters
        }
    }
    ...
    @discardableResult public func editPlace(
        id: UUID,
        name: String,
    ...
    public func removePlace(id: UUID, in context: ModelContext) {
