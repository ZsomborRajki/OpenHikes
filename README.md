...
- **Drawing a trail.** Every other trail in the app was drawn somewhere else — a recording is a walk that already happened, a GPX is a file somebody sent, a listing is somebody else's route. This is the one you make. Tap anywhere on the map and it tells you what is there and asks what you want it to be: the start, the far end, a stop along the way, or simply a place worth marking. Nothing changes until you answer, so looking at the map costs nothing. Each stretch between two points follows the paths OpenStreetMap has mapped, so the line goes where you could actually walk and the length is the length of the walk rather than of the line between the taps; a switch turns that off when you mean a straight line, and where nothing is mapped — or where OpenStreetMap is too busy to answer — the stretch stays straight, says so, and never stops you drawing. The route reads as a list of stops — where it starts, what it passes through, where it ends — joined by a line, with a handle on every row so any of them can be dragged somewhere else in the walk, and *Add Stop* under them whether or not anything is drawn yet. Each row says what is *there*: tap a spot on the map and its address appears a moment later, or tap the row itself to search for somewhere by name and put it in. What comes out is an ordinary hike, so it can be walked, styled, exported as GPX and taken offline like any other. A half-finished trail survives the app being closed — on this device, which is the only one that has any business holding an unfinished line — and coming back to it brings the map to where your points are.
- **Places along a trail.** The spring on the climb, the hut at the saddle, the junction you will otherwise walk past. Mark one from the map, from where you are standing or from a place you searched for, give it a symbol and a note, and drag its pin when you want it somewhere else. They are listed in the order you will walk past them rather than the order you marked them, so bending the route past the spring moves it up the list on its own. They are saved with the trail, sync with it, and travel in and out as ordinary GPX waypoints — which also means a file somebody else marked up arrives with their huts and springs on it rather than as a bare line. You do not have to know where they are, either: *Search this area* asks OpenStreetMap what is near the line you are drawing and offers it — the waterfall a hundred metres off the path, the spring on the climb, the hut at the saddle — ranked by how close it is to your route rather than to the middle of the screen. Most of them have no name at all, so each is offered by what it is and how far along the walk it sits, and nothing is added to your trail until you tap one.
