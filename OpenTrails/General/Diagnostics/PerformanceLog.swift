...
//  `xcrun simctl get_app_container booted tappium.com.OpenTrails data`, which
...
nonisolated final class PerformanceLog: Sendable {
    ...
    private static let logger = Logger(subsystem: "OpenTrails", category: "PerformanceLog")
    ...
    private let queue = DispatchQueue(label: "OpenTrails.PerformanceLog", qos: .utility)
    ...
    private func sampleResources() {
        ...
        record(
            ...
            name: "Process",
            value: footprint,
            detail: "cpu_s=\(sample.cpuSeconds)"
        )
    }
