...
UI-test app launches pass `--ui-testing`, which selects an in-memory SwiftData store and isolated `UserDefaults`. Additional launch arguments can expand the sheet (`--ui-test-expanded-sheet`), enable real simulator Core Location (`--ui-test-enable-location`), import a bundled GPX fixture (`--ui-test-import-gpx=ThumseeLoopFast`), match against a bundled trail graph instead of Overpass (`--ui-test-trail-graph=ThumseeRidgePath`), give the tile cache its own empty storage root and no network monitor so every tile is a genuine miss (`--ui-test-offline`), or open a performance log for a named scenario (`--ui-test-performance-log=<scenario>`). XCUITest drives static locations through `XCUIDevice.shared.location`; longer route playback remains available through `Scripts/simulate-hike.sh`. `Scripts/run-ui-tests.sh` boots one UI test (`--test`), one class (`--suite … --all`) or every functional class (`--all`) against a named simulator; `--list` prints them as `Class/method`. Fixtures, launch helpers and gestures shared by the bundle live in `OpenHikesUITests/UITestSupport.swift` — reuse them rather than re-deriving a coordinate or a wait.
...
Neither the functional UI automation nor the performance suite runs in CI — both drive a booted simulator through real gestures and timing-sensitive waits, which a shared runner makes slow and flaky, and the performance suite needs stable hardware for its numbers to mean anything. `AccessibilityUITests` is the exception: the `accessibility-ui-tests` job runs it through `Scripts/run-ui-tests.sh --suite AccessibilityUITests --all --retry`, granting location with `simctl privacy` up front so the one location-dependent test never sees a permission alert. That job is `continue-on-error: true` until its flake rate is known, so a red run reports without blocking. `PerformanceUITests` is listed in `skippedTests` for `OpenHikes.xctestplan`, so `xcodebuild test -scheme OpenHikes` does not pay for it; the `OpenHikesUI` scheme autocreates its plan and therefore still sees it, which is how `Scripts/run-performance-tests.sh` selects it. `Scripts/run-ui-tests.sh` names its functional classes explicitly — today `OpenHikesUITests` and `AccessibilityUITests` — for the same reason, so a new class has to be added to that list to be reachable through the script, and its `measurement_tests` list keeps `testLaunchPerformance` out of `--all` while still allowing `--test testLaunchPerformance`. A new UI test class added to that bundle inherits none of this and must be placed on the correct side of it.
