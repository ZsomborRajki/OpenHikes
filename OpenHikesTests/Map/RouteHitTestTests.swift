//
//  RouteHitTestTests.swift
//  OpenHikesTests
//
//  Which line a thumb landed on, without a map in the room.
//
...
//  test can hold. `MapCoordinatorTests+RouteTap.swift` and
//  `MapCoordinatorTests+CommunityRoutes.swift` cover the half that needs a
//  real `MKMapView` — that the lines are installed and that a tap reaches
//  this at all.
//
...
struct RouteHitTestTests {
    ...
    func aTapOnTheLineHits() {
        let index = RouteHitTest.nearest(
    ...
    func nothingDrawnIsNothingHit() {
        #expect(
            RouteHitTest.nearest(
                to: CGPoint(x: 10, y: 10),
                among: [],
                tolerance: Self.tolerance
            ) == nil
        )
        #expect(RouteHitTest.distance(from: CGPoint(x: 10, y: 10), to: []) == nil)
    }
}
