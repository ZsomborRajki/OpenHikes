...
The tree holds 163 first-party Swift files and 37,044 lines of shipping app
code, plus 3,948 in the shared package, 878 in the widget, and 32,415 lines of
...
What holds it back is no longer a defect list — there are no open findings — but
the gaps under "Test and project hygiene" below, and one thing that matters more
than the rest: the measurement apparatus has grown to roughly 3,900 lines of
diagnostics and scripts against 41,900 of product, and nothing runs the suite
that justifies it on a schedule.

Per domain, for calibration: modernity 9, tiles 8.5, photos/shared/widget 8.5,
map & app composition 8, hikes/data/sync 8, build & CI 8, recording 8,
test suite 7.5.

## Open findings

None. Every defect this review found has been fixed or disproven; the two that
turned out to be false — the bare `Task {}` sites and the `development`
`aps-environment` — are recorded under "Settled decisions" in
`.github/copilot-instructions.md` so they are not raised a third time.
...
| App and widget unit tests | 1,041 tests in 114 suites in `OpenHikesTests`, plus 23 in 3 suites in `OpenWidgetTests`, all passing |
...
| iOS build | Debug build for the simulator passes with Swift warnings as errors. Release not re-run this pass |
